...
//
#else /* gnu/linux ld */ // TM:and mingw/msys
...
typedef struct {
	...
	//Display *dpy;
	HWND win;
	HWND parentXwindow; // TODO:rename
	//Visual *visual;
	//long event_mask;
	//Atom DrawController;
#endif /* _WIN32 */
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_CreamMachineUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_CreamMachineUI* ui = (gx_CreamMachineUI*)malloc(sizeof(gx_CreamMachineUI));
deb("instantiate1");
	...
	ui->controls[0] = (gx_controller) {{1.0, 1.0, 0.0, 1.0, 1.0}, {40, 60, 61, 61}, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {310, 60, 61, 61}, false,"LEVEL", KNOB, LEVEL};
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {380, 60, 61, 61}, false,"BASS", KNOB, BASS};
	ui->controls[3] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {450, 60, 61, 61}, false,"TREBLE", KNOB, TREBLE};
	ui->controls[4] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {520, 60, 61, 61}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
...
static void cleanup(LV2UI_Handle handle) {
	gx_CreamMachineUI* ui = (gx_CreamMachineUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_CreamMachineUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_CreamMachineUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_CreamMachineUI *ui, gx_controller* controller) {
...
static void _expose(gx_CreamMachineUI *ui) {
	...
	cairo_move_to (ui->cr, ((double)(ui->width/1.3)/ui->rescale.x-(extents.width)/2.0),
	  (double)(ui->height/5.0)/ui->rescale.y-extents.height);
...
#ifdef _WIN32
    RECT rect;
    GetClientRect(ui->parentXwindow, &rect);
...
static void check_value_changed(gx_CreamMachineUI *ui, int i, float* value) {
...
static void check_is_active(gx_CreamMachineUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_CreamMachineUI *ui) {
...
static bool get_active_controller_num(gx_CreamMachineUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void enum_event(gx_CreamMachineUI *ui, int i) {
	float value;
...
static void switch_event(gx_CreamMachineUI *ui, int i) {
